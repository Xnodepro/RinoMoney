    ...
    class Money
    {
        ...
        public void INI()
        {
            try
            {
                ...
                Program._driver = new ChromeDriver(driverService);
                Program._driver.Navigate().GoToUrl("https://cs.money/ru");
                MessageBox.Show("22");
                ...
                new System.Threading.Thread(delegate () { try { TimeToSaveFile(); } catch (Exception ex) { } }).Start();
                new System.Threading.Thread(delegate () { try { Get(); } catch (Exception ex) { } }).Start();

                Program._driver.Navigate().GoToUrl("https://cs.money/ru");
                Program._driver.Manage().Window.Position = new Point(0, 0);

                var _cookies1 = Program._driver.Manage().Cookies.AllCookies;
                        ...
                        while (true)
                        {
                            try
                            {
                                if (websocket.State.ToString() == "Closing" || websocket.State.ToString() == "Closed")
                                {
                                    ...
                                    Thread.Sleep(10000);
                                    var _cookies2 = Program._driver.Manage().Cookies.AllCookies;
        ...
        private void websocket_Opened(object sender, WebSocket4Net.MessageReceivedEventArgs e)
        {
            try
            {
                ...
                if (da.Event.Value == "update_list_change_price")
                {
                    var countAddItems = da.data.Count;
                    Program.Mess.Enqueue(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff") + "|[ListChange]Изменило цену, предметов :" + countAddItems);
                    CheckItemsFromUpdate(da);
                }
        ...
        private void Get()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    var _cookies = Program._driver.Manage().Cookies.AllCookies;
        ...
        private void ClickItem(dynamic json)
        {
            ...
            try
            {
                ...
                foreach (var item in da)
                {
                    ...
                    if (item.e != null)
                    {
                        ...
                        catch (Exception ex) { Program.Mess.Enqueue("БОТ[" + ID + "] " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff") + "|[М]Ошибка1 :" + ex.Message); }
                    }
                    ...
                    if (ItemFind == null)
                    {
                        ...
                        Program.MessageDateBase.Enqueue("БОТ[" + b.ToString() + "] " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff") + "|Добавил предмет:" + DFM.Name + "|Цена:" + DFM.Price.ToString());
                    }
                    else
                    {
                        if (ItemFind.Price > p)
                        {
                            ...
                            Program.MessageDateBase.Enqueue("БОТ[" + b.ToString() + "] " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff") + "|Изменил предмет:" + Program.DataMoney[ItemIndex].Name + "|ЦенаOLD:" + Program.DataMoney[ItemIndex].Price.ToString() + "NewPrice:" + p.ToString());
            ...
            {
                Program.Mess.Enqueue("БОТ[" + ID + "] " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff") + "|[М]Ошибка2 :" + ex.Message);
            }
        }
        ...
        private void CheckItemsFromUpdate(dynamic json)
        {
            try
            {
                var da = json;
                ...
                foreach (var item in da.data)
                {
                    ...
                    try
                    {
                        ...
                        var price = item.First.price.Value;

                        int ItemIndex = Program.DataMoney.FindIndex(N => (N.Name.Replace(" ", "") == name.Replace(" ", "")));
                        Program.MessageDateBase.Enqueue("БОТ[" + ID + "] " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff") + "|[ListChange]Изменил предмет:" + Program.DataMoney[ItemIndex].Name + "|ЦенаOLD:" + Program.DataMoney[ItemIndex].Price.ToString() + "NewPrice:" + price.ToString());
                        Program.DataMoney[ItemIndex].Price = price;
                        Program.DataMoney[ItemIndex].Dt = DateTime.Now;
                    }
                    catch (Exception ex)
                    {
                        Program.Mess.Enqueue(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff") + "|[ListChange] :" + ex.Message);
                    }


                }
            }
            catch (Exception ex) { DateTime.Now.ToString(("yyyy-MM-dd HH:mm:ss:fff") + "|Ошибка2 :" + ex.Message); }
        ...
        private void TimeToSaveFile()
        {
            while (true)
            {
                ...
                {
                    Thread.Sleep(100000);
                    string json = JsonConvert.SerializeObject(Program.DataMoney);
                    File.WriteAllText("dataBaseMoney.txt", json);
                }
                catch (Exception ex) { }
            }
